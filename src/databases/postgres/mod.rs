...
impl Postgres {
    ...
    {
        return sqlx::query_as_with::<<Postgres as Executor>::T, O, _>(AssertSqlSafe(sql), arguments)
            .fetch_one(&self.db)
            .await
            .map_err(|e| e.into());
    }
    ...
    async fn execute_query<'q>(&'q self, sql: String, arguments: <<Postgres as Executor>::T as sqlx::Database>::Arguments) -> Result<impl QueryResult> {
        return sqlx::query_with::<<Postgres as Executor>::T, _>(AssertSqlSafe(sql), arguments)
            .execute(&self.db)
            .await
            .map_err(|e| e.into())
            .map(|result| {
